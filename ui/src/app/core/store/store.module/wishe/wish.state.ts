...
export class WishState {
  ...
  getAllCoachings(
    ctx: StateContext<WishStateModel>,
    { payload }: GetAllWishesCoaching
  ) {
    return this.service.getAllWishesCoachings(payload).pipe(
      tap((wish: Wish[]) => {
        ctx.setState(
          patch({
            coachings: wish
  ...
  getAllArticles(
    ctx: StateContext<WishStateModel>,
    { payload }: GetAllWishesArticles
  ) {
    return this.service.getAllWishesArticles(payload).pipe(
      tap((wish: Wish[]) => {
        ctx.setState(
          patch({
            articles: wish
  ...
  addWishCoaching(
    { getState, patchState, setState }: StateContext<WishStateModel>,
    { payload }: AddWishCoaching
  ) {
    return this.service.addWishCoaching(payload).pipe(
      tap((result: Wish) => {
        setState(
          patch({
  ...
  addWishArticle(
    { getState, patchState, setState }: StateContext<WishStateModel>,
    { payload }: AddWishArticle
  ) {
    return this.service.addWishArticle(payload).pipe(
      tap((result: Wish) => {
        setState(
          patch({
  ...
  deleteWishArticle(
    { getState, setState }: StateContext<WishStateModel>,
    { id }: DeleteWishArticle
  ) {
    return this.service.deleteWish(id).pipe(
      tap(() => {
        setState(
          patch({
  ...
  deleteWishByIdArticle(
    { getState, setState }: StateContext<WishStateModel>,
    { id }: DeleteWishByIdArticle
  ) {
    const wish = getState().articles.find(w => w.article._id === id);
    return this.service.deleteWish(wish._id).pipe(
      tap(() => {
        setState(
          patch({
